constants={
'normal':"Normal 5v5",
'aram':"Howling Abyss",
'rankedTeam':"Ranked Team 5v5",
'soloQ':"Ranked Solo 5v5",
'lolkingUrl': "http://www.lolking.net/summoner/las/235830",
'startTrim': "<!-- MATCH HISTORY -->",
'endTrim': "<!-- MASTERIES -->",
'detailsExtended': 'match_details_extended',
'dataGameIdClass': 'data-game-id',
'winText': "Win" ,
}
champions=[
	"Aatrox", "Ahri", "Akali", "Alistar", "Amumu", "Anivia", "Annie", 
	"Ashe", "Blitzcrank", "Brand", "Caitlyn", "Cassiopeia", "Cho'gath", "Corki", "Diana", 
	"Dr. Mundo", "Draven", "Evelynn", "Ezreal", "Fiddlesticks", "Fiora", "Fizz", "Galio",
	"Gangplank", "Garen", "Gragas", "Graves", "Hecarim", "Heimerdinger", "Irelia", 
	"Janna", "Jarvan IV", "Jayce", "Jax", "Jinx", "Karma", "Karthus", "Kassadin", "Katarina", "Kayle",
	"Kennen", "Kha\'Zix", "Kog'Maw", "LeBlanc", "Lee Sin", "Leona", "Lissandra", "Lucian", "Lulu", "Lux", "Malphite", 
	"Malzahar", "Maokai", "Master Yi", "Miss Fortune", "Mordekaiser", "Morgana", "Nami",
	"Nasus", "Nautilus", "Nidalee", "Nocturne", "Nunu", "Olaf", "Orianna", "Pantheon",
	"Poppy","Quinn", "Rammus", "Renekton", "Rengar", "Riven", "Rumble", "Ryze", "Sejuani", "Shaco",
	"Shen", "Shyvana", "Singed", "Sion", "Sivir", "Skarner", "Sona", "Soraka", "Swain", "Syndra", 
	"Talon", "Taric", "Teemo", "Tristana", "Trundle", "Trydamere", "Twisted Fate", "Twitch",
	"Udyr", "Urgot", "Varus", "Vayne", "Veigar", "Vi", "Viktor", "Vladimir", "Volibear", 
	"Warwick", "Wukong", "Xerath", "Xin Zhao", "Yorick", "Zac", "Ziggs", "Zilean", "Zyra"
]

